 ...
 * four contrast failures on the live page that the static gate passed, which is
 * the reason this exists. axe is a JavaScript engine, so this one gate is Node
 * even though the rest of the build is Python.
 *
 *   node scripts/axe-audit.mjs [output-dir]
 *
 * Exits non-zero on any violation at the configured conformance level. Serves
 * public/ over loopback rather than file:// so asset requests behave as they
 * do in production — an unstyled page has no contrast failures and would pass
...
const DIST = process.argv[2] ?? 'public';
  ...
  if (broken.length) {
    found.unshift({
      ...
      violation: {
        ...
        helpUrl: 'https://github.com/netresearch/agent-engineering/blob/main/scripts/axe-audit.mjs',
...
async function main() {
  ...
  if (routes.length === 0) throw new Error(`no pages found in ${DIST} — is public/index.html committed?`);
